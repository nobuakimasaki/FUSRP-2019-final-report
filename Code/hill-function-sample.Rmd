...
design_matrix <- read.csv("hill-function-sample_10_4.csv")
...
ggplot(design_matrix_lines, aes(x = n, y = K, color = line)) + geom_tile() + geom_vline(xintercept = 3) + geom_hline(yintercept = 75) + scale_colour_manual(values = c("red", "blue"), name = "Lines") + scale_x_continuous(breaks = seq(0,20)) + labs(title = "Lines of Minimum Error from Invariants", x = "Tested n", y = "Tested K") + theme(text = element_text(size=20), axis.text.x = element_text(angle=90, hjust=1), plot.title = element_text(hjust = 0.5)) + facet_wrap(vars(index))
```
