...
```{r}
#This code is used to create plots of the variance invariant lines for MA across different parameter values (the data is named mass-action-batch.csv). Note that x and y in the code refer to x_2 and x_1 respectively in the paper.
...
#Applying calculate_error_iter_MA to the design_matrix.
set.seed(17)
...
#Reading csv (uncomment next line to read)
...
```{r}
#if x2 -> x1
#This chunk produces the data for P5, where we plot the invariant line assuming that x_2 produces x_1. To do this, we reread the MA files and name x, y and y, x to change how we calculate the invariant
setwd("MA_stationary_distros")
...
#Naming columns (notice how we change order of x and y)
...
#Coercing design matrix to list
d_ <- list()
...
```{r}
#n = 0
#This chunk produces the data for P7, where we plot the invariant line when the exponent is 0.

#We first change the directory to where all of the MA stationary distributions with n = 0 is located
setwd("n_0")

#Reading all of the tables
MA_tbl <- 
    list.files(pattern = "*.csv") %>% 
    map_df(~fread(.))
...
#The rest is the same as chunk 4
n <- seq(-1, 6, 0.1)
x_bar <- 50
y_bar <- 50
gamma <- 0.1
