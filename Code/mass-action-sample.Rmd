...
design_matrix <- read.csv("mass-action-sample.csv")
design_matrix_cred_ivl <- design_matrix %>%
  group_by(n) %>%
  summarise(error_AB_25 = quantile(error_AB, 0.025), error_BC_25 = quantile(error_BC, 0.025), error_AB_975 = quantile(error_AB, 0.975), error_BC_975 = quantile(error_BC, 0.975))

ggplot(design_matrix, aes(n)) + geom_ribbon(data = design_matrix_cred_ivl, alpha = 0.1, aes(ymax = error_AB_975, ymin = error_AB_25))
```
