...
```{r}
#This code is used to create plots of the derivates for the MA invariant lines across different parameter values (the data is named mass-action-statistics.csv). Note that x and y in the code refer to x_2 and x_1 respectively in the paper.
...
#This function is used to iterate across different test n as well as different stationary
#distributions generated from different true n. It calls the previous function
#for every iteration, returning a list containing the 2nd and 3rd degree invariant errors for all
#tested n for every stationary distribution. 
#design_matrix denotes a list with test n values to evaluate the invariant errors at.
...
#Helper function for generating a Poisson distribution. Used to calculate the KLD and BC between the stationary distribution and the Poisson distribution with the same mean
...
#Calculates the KLD between the stationary distribution and the Poisson distribution with the same mean
...
#Calculates the BC between the stationary distribution and the Poisson distribution with the same mean
...
#Currently does not work (should be rewritten)
Fc <- function(sta){
...
#Calculates the numerical slope of the invariant at the true value of n
...
#loading all of the files in MA_stationary_distros
MA_tbl <- 
...
```{r}
#Creating the design matrix (list of parameter values that the stationary distributions were generated from)
...
#Applying calculate_error_iter_MA_batch to the design_matrix.
set.seed(17)
...
```{r}
#This chunk is used to create P4 from the data generated in the previous chunk
library(latex2exp)
